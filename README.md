...
## Manhattan plot
A Manhattan plot is an illustraion that plots inidividual SNPs found in a dataset, where autosomal chromsomes (1-22) are represented on the x-axis and -log10(p-value) on the y-axis. They are called "Manhattan" as each plot represents a skyline where the taller the chromosome's corresponding SNPs are plotted, the more statistically significant it is. Therefore, it can be concluded that the chromosome possesses genes that exhibit a strong SNP signal in coronary artery disease.
```r
